...
export default function InventoryPage() {
  ...
  const fetchPriceHistory = async (ingredientId: string) => {
    try {
      const res = await fetch(`/api/v1/ingredients/${ingredientId}/price-history`);
      if (res.ok) {
        const data = await res.json();
  ...
  const fetchMenuRecipe = async (menuId: string) => {
    try {
      const res = await fetch(`/api/v1/menus/${menuId}/recipe`);
      if (res.ok) {
        const data = await res.json();
        setRecipeLines(data);
      }
  ...
  const submitPriceUpdate = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`/api/v1/ingredients/${activeIngredient.id}/price-history`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
  ...
  const submitEditConfig = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`/api/v1/ingredients/${activeIngredient.id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          name: editName,
  ...
  const submitEditMenu = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const resMenu = await fetch(`/api/v1/menus/${activeMenu.id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          name: menuName,
      ...
      const resRecipe = await fetch(`/api/v1/menus/${activeMenu.id}/recipe`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          recipe: recipeLines
